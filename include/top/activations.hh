/**
 * \file activations.hh
 *  Contains all activation functions and
 *  auxiliary functions (such as derivative). <br>
 *  Contains a global table of functions. <br>
 *  The contents of the table represent the objects of
 * each activation function, which can be retrieved by name.
 * Defined in file activations_table.cc
 */
 ...
 * Each function must exist in at least one instance to be used by the model
 * processor <br> therefore the existence of this table is necessary
 */
struct Table {
  std::vector<fptr>
      table;  //< table of pointers to objects of each activation function
  /**
   * constructor <br>
  ...
  /**
   * returns a pointer to the activation function whose identifier matches the
   * one passed \param id activation function id \return pointer to the
   * corresponding function
   */
  fptr get_by_id(const size_t id);
  /**
   * returns a pointer to the activation function whose name matches the one
   * passed \param name name of the function \return pointer to the
   * corresponding function
   */
...
extern Table
    table;  //< table of activation functions. Defined in activations_table.cc

/**
 * calculates the first derivative for the passed activation function with a
 * certain accuracy \param neuron the point at which it is necessary to
 * calculate the derivative \param f activation function for which the
 * derivative is calculated \param accuracy the accuracy with which you need to
 * find the derivative \return derivative value at a point neuron.get_value()
 */
