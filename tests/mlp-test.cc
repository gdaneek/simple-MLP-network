...
TEST_CASE("Check for error in assigning layers of different sizes") {
	...
	CHECK_ERROR_MSG(l1 = l2, "The size of lhs layer and the size of the rhs layer do not match");
}
...
TEST_CASE("Test errors during creation connections for layer size 0") {
	...
	CHECK_ERROR_MSG(MLPLink(0,7), "trying to create links for a layer with size 0");
...
TEST_CASE("Test netMLP feedforward and response") {
	MLPModelSaver mds;
	std::vector<std::tuple<size_t, activations::fptr>> layers{{4, activations::table.get_by_name("ReLU")},
															  {2, activations::table.get_by_name("softmax")}};
	weight_vector weights{0.5, 0.1, 0.2, 0.15, 0.7, 0.85, 0.55, 0.4};
	...
	std::vector<neuron_t> out_layer_values_before_softmax{1.1875+shifts[shifts.size()-2], 0.6875+*--shifts.end()};

	REQUIRE(std::get<size_t>(res) == 0);	// first neuron in out_layer_values_before_softmax is max

	neuron_t exp_sum{};
	for(auto& val : out_layer_values_before_softmax)
		exp_sum += std::exp(val);

	std::vector<neuron_t> out_layer_values_after_softmax;
	for(size_t i{};i < out_layer_values_before_softmax.size(); ++i)
		out_layer_values_after_softmax.push_back(std::exp(out_layer_values_before_softmax[i])/exp_sum);
