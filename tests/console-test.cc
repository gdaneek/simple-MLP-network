...
TEST_CASE("Test add no args") {
  ...
  cons.add_layer({});
  std::string expected{
      "Empty net is initialized add_layer command requires at least 2 "
      "argument"};
...
TEST_CASE("Test add invalid activation") {
  ...
  std::string expected{
      "Empty net is initialized Failed process command: Requested activation "
...
TEST_CASE("Test load valid") {
  ...
  std::filesystem::remove("test_load.mlp");
}
