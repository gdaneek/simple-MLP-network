...
std::string MLPModelSaver::save_net_to_file(NetMLP& net, std::string path,
                                            std::string msg) {
  std::ofstream fout(((path == "") ? make_filename(net) : path),
  ...
  for (uint64_t data :
       std::vector<uint64_t>{file_signature, net.size(), weight_count(net)})
    byte_writer(data, fout);

  for (size_t i{0}; i < net.size(); i++) {
    byte_writer((uint64_t)net.layers[i].size(), fout);
    byte_writer((uint64_t)activations::table.id_by_ptr(net.activations[i]),
                fout);
  }

  for (auto link : net.links)
    for (auto weight_v : link) byte_writer(weight_v, fout);

  for (auto layer : net.layers)
    // for(auto neuron : layer)
    for (size_t i{0}; i < layer.size(); i++)
      byte_writer(static_cast<NeuronMLP>(layer[i]).shift, fout);
  byte_writer(model_end_lb, fout);
  for (char c : msg) byte_writer(c, fout);
  byte_writer('\0', fout);
  fout.close();
  return (path == "") ? make_filename(net) : path;
}

NetMLP MLPModelSaver::netmaker(
    std::vector<std::tuple<size_t, activations::fptr>>& layers,
    std::vector<weight>& weights, std::vector<neuron_t>& shifts) {
  NetMLP net(std::get<size_t>(*layers.begin()),
             std::get<activations::fptr>(*layers.begin()),
             std::get<size_t>(*--layers.end()),
             std::get<activations::fptr>(*--layers.end()));

  for (size_t i{1}; i < layers.size() - 1; i++)
    net.add(std::get<size_t>(layers[i]),
            std::get<activations::fptr>(layers[i]));
  try {
    net.make();
  } catch (...) {
    throw std::runtime_error{
        "Model builder: This model uses invalid layers. Build failed\n"};
  }
  try {
    ...
    throw std::runtime_error{
        "Model builder: missing weights or offsets. Initialization of "
        "trainable parameters failed\n"};
  }
  return net;
}

std::tuple<NetMLP, std::string> MLPModelSaver::upload_net_from_file(
    std::string path) {
  std::ifstream fin(path, std::ios::binary | std::ios::in);
  auto byte_reader{[](auto& value, std::ifstream& f) {
  ...
  std::vector<std::tuple<size_t, activations::fptr>> layers(
      layers_num);  // число нейронов + активация

  for (size_t i{0}; i < layers_num;
       shifts_num += std::get<size_t>(layers[i]), i++) {
    uint64_t neurons_num, activation;
    byte_reader(neurons_num, fin);
    byte_reader(activation, fin);
    try {
      layers[i] =
          std::make_pair(neurons_num, activations::table.get_by_id(activation));
...
void MLPModelSaver::show_model_info(NetMLP& net, std::string msg,
                                    std::ostream& ostream) const {
  ostream << "\033[95mNet information: \033[39m\n";
  ostream << "\033[96mNumber of layers: \033[39m" << net.size() << std::endl;
  for (size_t i{0}; i < net.size(); i++)
    ostream << "\t\033[92mLayer " << i << ": \033[39m" << net.layers[i].size()
            << " neurons with "
            << activations::table.name_by_ptr(net.activations[i]) << std::endl;
  ostream << "\033[96mMessage from file:\033[39m"
          << ((msg == "") ? "" : "\n" + msg) << std::endl;
}
