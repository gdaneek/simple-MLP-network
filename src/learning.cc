...
#include "mlp.hh"  // delete after tests, or not
...
std::vector<neuron_t> vectorize_label(LabelType& label) {
  ...
  //     res.push_back(label);                           // maybe change label
  //     type to int (neuronval)
  int count_of_figures =
      7;  // take from
          // somewhere!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
// void NetMLP::backprop(const std::vector<double>& target, double
// learning_rate) {
//     size_t last = layers.size() - 1;
//     weight_vector delta_previous(layers.back().size());
//     for (size_t j{0}; j < layers.back().size(); j++) {
//         delta_previous[j] = -2 * learning_rate * layers.back()[j].get_value()
//         * (1 - layers.back()[j].get_value()) * (target[j] -
//         layers.back()[j].get_value());         // j ????
//     }
//     weight_vector delta;
...
//             }
//             delta[k] = 2 * learning_rate * layers[j][k].get_value() * (1 -
//             layers[j][k].get_value()) * sum_d;
//         }
...
//     for (size_t j{0}; j < layers.back().size(); j++) {
//         delta_previous[j] = (-1) * layers.back()[j].get_value() * (1 -
//         layers.back()[j].get_value()) * (target[j] -
//         layers.back()[j].get_value());
//     }
...
//                 sum_d += delta_previous[t] * links[j].get_weight(k, t);
//             }
//             delta[k] = layers[j][k].get_value() * (1 -
//             layers[j][k].get_value()) * sum_d;
//         }
//         // change w with delta_previous (from k to t)
//         for (size_t k{0}; k < layers[j].size(); k++) {
...
//             for (size_t t{0}; t < links.size(); t++) {
//                 // double dw = delta_previous[t] * layers[j][k].get_value() *
//                 learning_rate; double dw = delta[k] *
//                 layers[j][k].get_value() * learning_rate; weight new_weight =
//                 links[j].get_weight(k, t) - dw; links[j].set_weight(k, t,
//                 new_weight);
//                 // links[j].weights[k][t] += dw;
//             }
//         }
...
//             for (size_t t{0}; t < delta_previous.size(); t++) {
//                 double dw = delta_previous[t] * layers[j][k].get_value() *
//                 learning_rate; weight new_weight = links[j].get_weight(k, t)
//                 - dw; links[j].set_weight(k, t, new_weight);
//             }
//         }
//         // delta_previous = delta
//         delta_previous.clear();
...
void NetMLP::backprop(const std::vector<double>& target, double learning_rate) {
  ...
  double mse{0};
  for (size_t j{0}; j < layers.back().size(); j++) {
    mse += (target[j] - layers.back()[j].get_value()) *
           (target[j] - layers.back()[j].get_value());
  }
  ...
  for (size_t j{0}; j < layers.back().size(); j++) {
    // delta_previous[j] = err * activations::derivative(layers.back()[j],
    // *activations[last], 1e-6);
    delta_previous[j] =
        (target[j] - layers.back()[j].get_value()) *
        activations::derivative(layers.back()[j], *activations[last], 1e-6);
    // delta_previous[j] = (-1) * (target[j] - layers.back()[j].get_value()) *
    // activations::derivative(layers.back()[j], *activations[last], 1e-6);
    // delta_previous[j] = (-1) * layers.back()[j].get_value() * (1 -
    // layers.back()[j].get_value()) * (target[j] -
    // layers.back()[j].get_value());
  ...
  for (int j{last - 1}; j >= 0; j--) {
    ...
    for (size_t k{0}; k < layers[j].size(); k++) {
      ...
      // delta[k] = layers[j][k].get_value() * (1 - layers[j][k].get_value()) *
      // sum_d;
      delta[k] =
          sum_d * activations::derivative(layers[j][k], *activations[j], 1e-6);
      // delta[k] = sum_d * ;
      // delta[k] = (-1) * sum_d * activations::derivative(layers[j][k],
      // *activations[j], 1e-6);
    }
    // change w with delta_previous (from k to t)
    for (size_t k{0}; k < layers[j].size(); k++) {
      ...
      for (size_t t{0}; t < delta_previous.size(); t++) {
        double dw =
            delta_previous[t] * layers[j][k].get_value() * learning_rate;
        // weight new_weight = links[j].get_weight(k, t) - dw;
        // links[j].set_weight(k, t, new_weight);
...
// // for tangh
// void NetMLP::backprop(const std::vector<double>& target, double
// learning_rate, double moment) {
//     // calc err---------------------
//     double mse{0};
//     for (size_t j{0}; j < layers.back().size(); j++) {
//         mse += (target[j] - layers.back()[j].get_value()) * (target[j] -
//         layers.back()[j].get_value());
//     }
//     mse /= layers.back().size();
//     std::cout << mse << std::endl;
...
//     for (size_t j{0}; j < layers.back().size(); j++) {
//         delta_previous[j] = (target[j] - layers.back()[j].get_value()) *
//         activations::derivative(layers.back()[j], *activations[last], 1e-6);
//         // delta_previous[j] = (-1) * layers.back()[j].get_value() * (1 -
//         layers.back()[j].get_value()) * (target[j] -
//         layers.back()[j].get_value());
//         // delta_previous[j] = layers.back()[j].get_value() * (1 -
//         layers.back()[j].get_value()) * (target[j] -
//         layers.back()[j].get_value());
//     }
...
//                 sum_d += delta_previous[t] * links[j].get_weight(t, k);
//             }
//             delta[k] = sum_d * (1 - layers[j][k].get_value() *
//             layers[j][k].get_value());
//         }
//         // change w with delta_previous (from k to t)
//         for (size_t k{0}; k < layers[j].size(); k++) {
//             layers[j][k].shift -= delta[k] * learning_rate;
...
//                 double dw = grad * learning_rate + moment *
//                 links[j].get_weight(t, k); weight new_weight =
//                 links[j].get_weight(t, k) - dw; links[j].set_weight(t, k,
//                 new_weight);
//             }
//         }
//         // delta_previous = delta
...
void train(NetMLP& net, std::string dir) {
  ...
  for (const auto& entry : fs::directory_iterator(dir)) {
    ...
    if (path.find("shape") != std::string::npos)
      shapes.insert(path);
    else
      labels.insert(path);
  }
  if (shapes.size() != labels.size())
    throw std::runtime_error{
        "The number of labels is not equal to the number of images\n"};
  // std::cout << "\n SHAPES: \n";
  // for(auto& s : shapes)std::cout << s << std::endl;
  // std::cout << "\n LABELS: \n";

  for (auto shapes_it{shapes.begin()}, labels_it{labels.begin()};
       labels_it != labels.end(); shapes_it++, labels_it++) {
    ...
    std::cout << std::get<size_t>(res) << " " << std::get<neuron_t>(res)
              << std::endl;
...
// std::tuple<size_t, size_t> test(NetMLP& net, std::string dir) { // ОБЩЕЕ
// ЧИСЛО ТЕСТОВ И СКОЛЬКО ИЗ НИХ ПРОЙДЕНЫ УСПЕШНО
